...
public class CmDealUtil {
    ...
    public static CmAccountRedis getCmAccount(Long customerId, String coinCode) {
        try (Jedis jedis = CmRedis.JEDIS_POOL.getResource()) {
            ...
            Cm4AccountService cmAccountService = (Cm4AccountService) SpringUtil.getBean("cm4AccountService");
            synchronized (customerId) {
                QueryFilter filter = new QueryFilter(Cm4Account.class);
                filter.addFilter("customerId=", customerId);
                filter.addFilter("coinCode=", coinCode);
                List<Cm4Account> cmAccountlist = cmAccountService.find(filter);

                if (null != cmAccountlist &&cmAccountlist.size()>0) {
                    Cm4Account binaryAccount=cmAccountlist.get(0);
                     ...
                     * 创建币账户
                     */
                    Cm4Account account = new  Cm4Account();
