...
public class RemoteCmAccountServiceImpl implements RemoteCmAccountService {
	...
	@Resource
    public Cm4AccountService cmAccountService;

    @Resource
    public Cm4AccountRecordService cmAccountRecordService;
    ...
    public Boolean initAccount(Long customerId) {
        ...
        // 查询该用户的二元期权账户数据
        QueryFilter accountQf = new QueryFilter(Cm4Account.class);
        accountQf.addFilter("customerId=", customerId);
        List<Cm4Account> accountList = cmAccountService.find(accountQf);
        if (accountList != null && accountList.size() > 0) {
            // 循环判断该账户在redis中是否存在，不存在添加，存在跳过
            try (Jedis jedis = CmRedis.JEDIS_POOL.getResource()){
                for (Cm4Account account : accountList) {
	...
	public FrontPage findMyAccountRecordList(Map<String, String> params) {
		...
		QueryFilter filterCmAccountRecord = new QueryFilter(Cm4AccountRecord.class);
		...
		List<Cm4AccountRecord> list = cmAccountRecordService.find(filterCmAccountRecord);
	...
	public List<CmAccountRedis> getAccoutList(Long customerId) {
		...
		try (Jedis jedis = CmRedis.JEDIS_POOL.getResource()) {
            ...
            QueryFilter qf = new QueryFilter(Cm4Account.class);
            qf.addFilter("customerId=", customerId);
            List<Cm4Account> list = cmAccountService.find(qf);
            List<CmAccountRedis> listaccount = new ArrayList<CmAccountRedis>();
            for (Cm4Account c : list) {
