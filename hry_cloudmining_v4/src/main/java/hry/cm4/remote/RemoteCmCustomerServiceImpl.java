...
import hry.cm4.remote.model.CmCustomerRemote;
import hry.cm4.remote.model.CmTeamlevelRemote;
...
public class RemoteCmCustomerServiceImpl implements RemoteCmCustomerService {
    ...
    @Resource
    private Cm4AccountRecordService cmAccountRecordService;
    @Resource
    ...
    public JsonResult getCustomerGrade(Map<String, String> params) {
        ...
        Cm4Customer cmCustomer = null;
        if (str != null && !str.equals("")) {
            cmCustomer = JSON.parseObject(str, Cm4Customer.class);
        } else {
    ...
    public FrontPage findCustomerTeamlist(Map<String, String> params) {
        ...
        List<Cm4Teamlevel> list = cmTeamlevelService.findTeamByCustomerId(customerId, 1);//查询直推
    ...
    public JsonResult getProfitStatistics(Map<String, String> params) {
        ...
        /**获取用户收益统计信息 */
        List<Cm4AccountRecord> list = cmAccountRecordService.getProfitStatistics(customerId, platCoin);
        ...
        if (list != null && list.size() > 0) {
            for (Cm4AccountRecord cmAccountRecord : list) {
        ...
        /**查询当前用户封顶奖励*/
        //获取封顶倍数
        Cm4Customer cmCustomer = cmCustomerService.getCustomerByCustomerId(customerId);
    ...
    public JsonResult getMyMiner(Map<String, String> params) {
        ...
        /**查询矿机信息*/
        List<Cm4CustomerMiner> cmCustomerMinerList = cmCustomerMinerService.getMyMinerCustomerId(customerId);
        ...
        if (cmCustomerMinerList != null && cmCustomerMinerList.size() > 0) {
            for (Cm4CustomerMiner cmCustomerMiner : cmCustomerMinerList) {
