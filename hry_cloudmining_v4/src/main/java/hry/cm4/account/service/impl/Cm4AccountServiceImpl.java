...
import hry.cm4.account.dao.Cm4AccountDao;
...
import hry.cm4.account.service.Cm4AccountService;
import hry.cm4.comparator.AccountaddComparator;
 ...
 * @Date :          2019-11-20 15:18:36  
 */
@Service("cm4AccountService")
public class Cm4AccountServiceImpl extends BaseServiceImpl<Cm4Account, Long> implements Cm4AccountService{
	
	@Resource(name="cm4AccountDao")
	@Override
	public void setDao(BaseDao<Cm4Account, Long> dao) {
		super.dao = dao;
	}

	@Resource
	private Cm4AccountRecordDao cmAccountRecordDao;
	...
	public void saveAccount(List<Accountadd> accountaddlist){
		...
		List<Cm4AccountRecord> listehar1=new ArrayList<Cm4AccountRecord>();
		for(Accountadd accountadd:accountaddlist){
			setaccountcoin.add(accountadd.getAccountId());
			Cm4AccountRecord contractedOneAccountRecord = createRecord(accountadd);
		...
		while (iteratorc.hasNext()) {
			...
			Cm4Account CmAccount = this.get(id);
			if(CmAccount != null){
		...
		if(null!=listd&&listd.size()>0){
			((Cm4AccountDao)dao).updateAccount(listd);
		}


	}
	public Cm4AccountRecord createRecord(Accountadd rccountadd){
		//资金池5(完成)，托管0(处理中)将来通过配置文件来获取判断
		Cm4AccountRecord CmAccountDetail = new Cm4AccountRecord();
