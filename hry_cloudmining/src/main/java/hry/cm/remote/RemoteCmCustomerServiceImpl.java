...
import hry.cm.remote.model.CmCustomerRemote;
import hry.cm.remote.model.CmTeamlevelRemote;
...
public class RemoteCmCustomerServiceImpl implements RemoteCmCustomerService {
	...
	@Resource
	private CmAccountRecordService cmAccountRecordService;
	@Resource
	...
	public JsonResult getCustomerGrade(Map<String, String> params) {
		...
		String str = redisService.get("CM:CUSTOMERGRADE_CUSTOMERID:"+customerId);
		CmCustomer cmCustomer = null;
		if(str!=null&&!str.equals("")){
			cmCustomer = JSON.parseObject(str, CmCustomer.class);
		}else{
	...
	public FrontPage findCustomerTeamlist(Map<String, String> params) {
		...
		List<CmTeamlevel> list = cmTeamlevelService.findTeamByCustomerId(customerId,1);//查询直推
	...
	public JsonResult getProfitStatistics(Map<String, String> params) {
		...
		Long customerId = Long.valueOf(params.get("customerId"));//用户id
		/**查询redis*/
		String str = redisService.get("CM:CUSTOMERGRADE_CUSTOMERID_PROFIT:"+customerId);
		if(str!=null&&!str.equals("")){
		...
		/**获取用户收益统计信息 */
		List<CmAccountRecord> list = cmAccountRecordService.getProfitStatistics(customerId, platCoin);
		...
		if(list!=null&&list.size()>0){
			for(CmAccountRecord cmAccountRecord:list){
		...
		/**查询当前用户封顶奖励*/
		//获取封顶倍数
		CmCustomer cmCustomer = cmCustomerService.getCustomerByCustomerId(customerId);
		BigDecimal cappingMultiple = cmCustomer.getCappingMultiple();//封顶倍数
		//查询用户今天产币信息
		...
		//保存信息到redis中，保存1小时
		redisService.save("CM:CUSTOMERGRADE_CUSTOMERID_PROFIT:"+customerId, JSON.toJSONString(paramResult),60*60*1);
	...
	public JsonResult getMyMiner(Map<String, String> params) {
		...
		/**查询redis*/
		String str = redisService.get("CM:CUSTOMERGRADE_CUSTOMERID_MYMINER:"+customerId);
		if(str!=null&&!str.equals("")){
			paramResult = JSON.parseObject(str,HashMap.class);
			return new JsonResult().setSuccess(true).setObj(paramResult);
		}
		...
		/**查询矿机信息*/
		List<CmCustomerMiner> cmCustomerMinerList = cmCustomerMinerService.getMyMinerCustomerId(customerId);
		...
		if(cmCustomerMinerList!=null&&cmCustomerMinerList.size()>0){
			for(CmCustomerMiner cmCustomerMiner:cmCustomerMinerList){
		...
		//保存信息到redis中，保存10分钟
		redisService.save("CM:CUSTOMERGRADE_CUSTOMERID_MYMINER:"+customerId, JSON.toJSONString(paramResult),60*10);
	...
	public JsonResult getMyMinercamps(Map<String, String> params) {
		...
		/**查询redis*/
		String str = redisService.get("CM:CUSTOMERGRADE_CUSTOMERID_MYMINERCAMPS:"+customerId);
		if(str!=null&&!str.equals("")){
			paramResult = JSON.parseObject(str,HashMap.class);
			return new JsonResult().setSuccess(true).setObj(paramResult);
		}
		...
		//保存信息到redis中，保存1小时
		redisService.save("CM:CUSTOMERGRADE_CUSTOMERID_MYMINERCAMPS:"+customerId, JSON.toJSONString(paramResult),60*60*1);
