...
import hry.cm5.deal.CmCustomerUtil;
...
public class RemoteCmAccountServiceImpl implements RemoteCmAccountService {
    ...
    @Resource
    public Cm5AccountRecordService cmAccountRecordService;
    ...
    public Boolean initAccount(Long customerId) {
        ...
        // 查询该用户的二元期权账户数据
        QueryFilter accountQf = new QueryFilter(Cm5Account.class);
        accountQf.addFilter("customerId=", customerId);
        List<Cm5Account> accountList = cmAccountService.find(accountQf);
        if (accountList != null && accountList.size() > 0) {
            // 循环判断该账户在redis中是否存在，不存在添加，存在跳过
            try (Jedis jedis = CmRedis.JEDIS_POOL.getResource()){
                for (Cm5Account account : accountList) {
	...
	public FrontPage findMyAccountRecordList(Map<String, String> params) {
		...
		QueryFilter filterCmAccountRecord = new QueryFilter(Cm5AccountRecord.class);
		...
		List<Cm5AccountRecord> list = cmAccountRecordService.find(filterCmAccountRecord);
	...
	public List<CmAccountRedis> getAccoutList(Long customerId) {
		...
		try (Jedis jedis = CmRedis.JEDIS_POOL.getResource()) {
            ...
            QueryFilter qf = new QueryFilter(Cm5Account.class);
            qf.addFilter("customerId=", customerId);
            List<Cm5Account> list = cmAccountService.find(qf);
            List<CmAccountRedis> listaccount = new ArrayList<CmAccountRedis>();
            for (Cm5Account c : list) {
